...
static int decode_avc2019(const uint8_t *header, const uint8_t *buf, int len){
    printf("size : %d\n",len);
    ...
    const AVC_HDR_t *hdr = (AVC_HDR_t *)header;  
    ...
    //return decode_MessageFrame(buf, dlen);
}
...
//  ros::Rate loop_rate(10);
    ...
    while (ros::ok()) {
        ...
        if (FD_ISSET(sock, &fds)) {
            nrecv = recvfrom(sock, _header, sizeof(AVC_HDR_t), 0, NULL, NULL);
            if (nrecv == -1) {
            ...
            const AVC_HDR_t *hdr = (AVC_HDR_t *)_header;  
            ...
            //uint8_t *buf = new uint8_t(dlen);
            //printf("buf size : %d\n",sizeof(buf));
            nrecv = recvfrom(sock, buf, dlen + sizeof(AVC_TAIL_t) , 0, NULL, NULL);
            if (nrecv == -1) {
                fprintf(stderr, "failed to recvfrom: %d\n", nrecv);
                continue;
                //close(sock);
            ...
            total_length += nrecv;              
            printf("header size + all length : %d\n",total_length);
            memmove(_buf+sizeof(AVC_HDR_t),buf,dlen + sizeof(AVC_TAIL_t));
            memmove(_buf,_header,sizeof(AVC_HDR_t));
            printf("size AVC_HDR_t : %d\n",sizeof(AVC_HDR_t));
            //memcpy(buf,_header,sizeof(AVC_HDR_t));



            ret = decode_avc2019(_header,_buf, total_length);
