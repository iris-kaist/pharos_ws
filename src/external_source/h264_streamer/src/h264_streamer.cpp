...
{
  H264StreamerUDPServer(unsigned short port)
...
{
  H264StreamerUDPClient(const std::string & host, unsigned short port)
  {
...
{
  H264StreamerTCPServer(unsigned short port)
...
{
  H264StreamerTCPClient(const std::string & host, unsigned short port)
...
public:
  ...
  {
    std::cout<<"UDP: "<<(conf.udp?"true":"false")
      <<"\nserver: "<<(conf.server?"true":"false")
      <<"\nport: "<<conf.port<<std::endl;
    if(conf.udp)
    {
      if(conf.server)
      {
